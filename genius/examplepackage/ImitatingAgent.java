 ...
 */
public class ImitatingAgent extends Agent {
	...
	private List<Bid> registeredBids;
	/**
	...
	public void init() {
		super.init();
		rand = new Random();
		registeredBids = new ArrayList<Bid>();
	...
	public Action chooseAction() {
		...
		try {
			if (actionOfPartner == null) {
				Bid bid = registeredBids.get(rand.nextInt(registeredBids.size()));
				previousBid = bid;
				action = new Offer(getAgentID(),bid);
			}
			if (actionOfPartner instanceof Offer) {
				registeredBids.add(((Offer) actionOfPartner).getBid());
				action = new Accept(getAgentID(), lastBid); 
			}
		} catch (Exception e) {
			e.printStackTrace();
			action = new Accept(getAgentID(), lastBid); // best guess if things go wrong.
		}
			return action;
		}
